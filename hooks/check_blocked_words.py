...
#
# 源文件: plugins/ios-blocked-words-check/skills/ios-blocked-words-check/scripts/check_blocked_words.py
# 源版本: ios-blocked-words-check@1.1.2
#
# 直接改本文件会在下次同步时被覆盖，且 CI 会拦。
# 要改规则请提 workshop，发 ios-blocked-words-check tag 后自动同步回这里。
# ══════════════════════════════════════════════════════════════════
...
#
# ⚠️ 改这里，不要改 ios-git-hooks。
#    ios-git-hooks 的 hooks/check_blocked_words.py 由 workshop CI 自动生成，
#    带 AUTO-GENERATED 头、禁止手改；发 ios-blocked-words-check tag 即自动同步过去。
#    该仓库的 hooks/check-sensitive-words 依赖 --diff-stdin，删改此参数会打断提交门禁。
#
# 用法:
#   python3 check_blocked_words.py [file1 file2 ...]  # 检查指定文件
#   python3 check_blocked_words.py --staged            # 检查 git staged 文件
#   python3 check_blocked_words.py --diff-stdin        # 从 stdin 读 git diff，只查新增行
...
BLOCKED_WORDS = [
    ...
    # demand: 索要礼物，DemandGift / demand_gift 等组合也违规
    {"word": "demand",           "mode": "compound"},
...
def check_diff_stdin(skip_comments: bool = False) -> list:
    ...
    pre-commit 门禁的主入口：只看本次改动引入的行，存量代码不报错。
...
def main():
    ...
    # --diff-stdin 直接从 diff 产出 violations，不经过文件列表
    if opts['diff_stdin']:
